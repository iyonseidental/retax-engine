...
/* =========================================================
 * 10-c. 주소 → 자치구 매핑 (조정대상지역 판정 연동)
 * ========================================================= */
{
  require(path.join(base, "js/app/address.js"));
  const A = R.Address;
  eq("서울 강남구", A.districtFromJuso("서울", "강남구"), "강남구");
  eq("서울특별시 서초구", A.districtFromJuso("서울특별시", "서초구"), "서초구");
  eq("서울 동작구", A.districtFromJuso("서울", "동작구"), "동작구");
  eq("서울 마포구 → 기타 서울", A.districtFromJuso("서울", "마포구"), "기타 서울");
  eq("경기 성남시 분당구 → 비규제(직접확인 안내)", A.districtFromJuso("경기", "성남시 분당구"), "비규제지역");
  eq("부산 해운대구 → 비규제", A.districtFromJuso("부산", "해운대구"), "비규제지역");
}
